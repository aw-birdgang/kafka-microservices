...
export class AuthService {
    ...
    async login(loginDto: LogInDto) {
        ...
        const passwordHash = await this.accountService.findPasswordHashById(userDto.id);
        ...
        await this.accountRegisterService.editToken(EditTokenParameter.from(userDto.id, ClientAccessType.USER, jWTokenDto.refreshToken));

        return jWTokenDto;
    }

    async refresh(requestRefreshToken: RequestRefreshTokenDto): Promise<JWTokenDto> {
        const userDto = await this.accountService.findByUsername(requestRefreshToken.username);
        if (isEmpty(userDto)) {
            throw new UnauthorizedException();
        }
        ...
        await this.accountRegisterService.editToken(EditTokenParameter.from(userDto.id, ClientAccessType.USER, jwtToken.refreshToken));
    ...
    async googleLoginCallBack(req: any) {
        ...
        await this.send<AuthDto>(AccountTcpCommands.ACCOUNT_playerUserGoogleLogin, request);

        const payload = { username: name, sub: email, roles: [] };
        const jwtToken = new JWTokenDto();
        jwtToken.accessToken = await this.generateAccessToken(payload);
        jwtToken.refreshToken = await this.generateRefreshToken(payload);
        await this.accountRegisterService.editToken(EditTokenParameter.from(email, ClientAccessType.USER, jwtToken.refreshToken));
