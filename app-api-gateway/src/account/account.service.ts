...
export class AccountService {
  ...
  findByUsername(username: string): Promise<UserDto> {
    ...
    return this.send<UserDto>(
      AccountTcpCommands.ACCOUNT_USER_BY_USERNAME,
      request,
    );
  }
  ...
  findByEmail(email: string): Promise<UserDto> {
    ...
    return this.send<UserDto>(
      AccountTcpCommands.ACCOUNT_USER_BY_EMAIL,
      request,
    );
  }
  ...
  findPasswordHashById(userId: number): Promise<string> {
    ...
    return this.send<string>(
      AccountTcpCommands.ACCOUNT_USER_PASSWORD_HASH_BY_ID,
      request,
    );
  }
  ...
  findUserProfileById(userId: number): Promise<UserProfileDto> {
    const request = TcpRequest.from<number>(userId);
    return this.send<UserProfileDto>(
      AccountTcpCommands.ACCOUNT_USER_PROFILE_BY_ID,
      request,
    );
  }
